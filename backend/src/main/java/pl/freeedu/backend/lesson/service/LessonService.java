...
public class LessonService {
	...
	public LessonService(LessonRepository lessonRepository,
						 GroupHasLessonRepository groupHasLessonRepository,
	...
	public Flux<LessonResponse> getLessons(String search, Integer groupId, Boolean status, String sort) {
		return Mono.fromCallable(() -> {
			...
			// To podejście proste i zgodne ze stylem projektu; można zmienić na zapytanie DB.
			...
			if (groupId != null) {
				...
				filtered = filtered.stream().filter(l -> lessonIdsForGroup.contains(l.getId())).collect(Collectors.toList());
			}

			// Sortowanie
			Comparator<Lesson> comparator = Comparator.comparing(Lesson::getCreatedAt, Comparator.nullsLast(Comparator.naturalOrder()));
			if ("date_desc".equalsIgnoreCase(sort) || sort == null) {
				comparator = Comparator.comparing(Lesson::getCreatedAt, Comparator.nullsLast(Comparator.reverseOrder()));
			} else if ("date_asc".equalsIgnoreCase(sort)) {
				comparator = Comparator.comparing(Lesson::getCreatedAt, Comparator.nullsLast(Comparator.naturalOrder()));
				...
				comparator = Comparator.comparing(Lesson::getTitle, Comparator.nullsLast(String.CASE_INSENSITIVE_ORDER)).reversed();
			}
	...
	public Mono<LessonResponse> createLesson(Mono<LessonRequest> requestMono) {
		return requestMono.flatMap(request -> securityService.getCurrentUserId().flatMap(teacherId ->
			Mono.fromCallable(() -> {
	...
	public Mono<Void> deleteLesson(Integer id) {
		return securityService.getCurrentUserId().flatMap(currentUserId ->
			Mono.fromCallable(() -> {
				...
				// kasowanie kaskadowe z tabeli group_has_lesson powinno być zapewnione przez migracje DB,
