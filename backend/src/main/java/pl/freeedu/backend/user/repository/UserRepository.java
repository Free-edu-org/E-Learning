...
public interface UserRepository extends JpaRepository<User, Integer> {
	...
	@Query("SELECT u.id as id, u.username as username, u.email as email, u.role as role, u.createdAt as createdAt, ug.id as groupId FROM User u JOIN UserInGroup uig ON u.id = uig.userId JOIN UserGroup ug ON uig.groupId = ug.id WHERE ug.teacherId = :teacherId AND u.role = :role")
