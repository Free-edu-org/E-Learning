...
function GroupChips({ groups }: { groups: Lesson["groups"] }) {
  ...
  return (
    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
      ...
      {overflow.length > 0 && (
        <Tooltip title={overflow.map((g) => g.name).join(", ")} arrow placement="top">
          <Chip
  ...
  return (
    <Card elevation={0} sx={panelGridCardSx}>
      <CardContent sx={{ ...panelGridCardContentSx, pb: 1 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5, ml: "26px" }}>
          {lesson.theme}
        </Typography>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
            {lesson.isActive ? (
              <Chip
                label="Aktywna"
                size="small"
                sx={{ bgcolor: "#1a1a2e", color: "#fff", fontWeight: 600, fontSize: "0.7rem", height: 22 }}
              />
            ) : (
              <Chip label="Nieaktywna" size="small" variant="outlined" sx={{ fontWeight: 600, fontSize: "0.7rem", height: 22 }} />
            )}
            <Switch size="small" checked={lesson.isActive} readOnly sx={{ pointerEvents: "none" }} />
          </Box>
        </Box>
